...
## Frontend auth flow (discovered 2026-06-13)
- **Auth is magic-link only.** Backend `AuthController` exposes just `POST /api/auth/magic-link/send`, `POST /api/auth/magic-link/verify`, `GET /api/auth/me`, `POST /api/auth/logout`. There is **no** `/api/auth/register` and **no** password login. Magic-link send auto-creates the user, so it serves as both signup and login. Don't wire frontend auth to endpoints that don't exist.
- **Magic-link verify URL convention:** the backend builds the link as `${FRONTEND_URL}/auth/verify?token=...`. So the frontend verify page must live at `app/auth/verify/page.tsx` (real path `/auth/verify`). Do NOT put it under the `(auth)` route group — route groups are stripped from the URL, so `(auth)/verify` resolves to `/verify` and magic links 404.
- **Magic-link email is a stub:** `apps/backend/src/libs/queue/processors/email.processor.ts` `sendMagicLink()` is a TODO that only logs — no email is sent. In dev the only way to get the link is from the email-worker logs. (The email processor is NOT on the NEVER-touch list; backend `auth/**` is.)
- **Frontend logging:** use `apps/frontend/lib/logger.ts` (`createLogger('scope')`) instead of `console.*` — `console` is intentional only inside `logger.ts` as the sink. `logger.error(message, error?, meta?)`, others are `(message, meta?)`.
- **Frontend is NOT linted by the gates:** the frontend `lint` npm script is a no-op echo, so `npm run lint` never runs ESLint over `apps/frontend`. Type safety is the only automated check (`tsc --noEmit` / `next build`). Run `npx tsc --noEmit` in `apps/frontend` to validate frontend changes.
