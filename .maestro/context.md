...
- **Auth is magic-link only.** The backend `AuthController` exposes only `magic-link/send`, `magic-link/verify`, `me`, `logout` — no password register/login. `sendMagicLink` auto-creates a user on first email, so signup and login are the same flow. Don't add password endpoints without explicit instruction (`apps/backend/src/auth/**` is on the never-touch list).
- **Magic-link URL is `/auth/verify`** (not `/verify`). `auth.service.ts` builds `${FRONTEND_URL}/auth/verify?token=...`. The frontend route lives at `apps/frontend/app/auth/verify/page.tsx` (outside the `(auth)` route group). If you move it back inside `(auth)`, the magic link will 404.
- The frontend `app/page.tsx` is an auth-aware redirect (→ `/dashboard` or `/login`), not a landing page. If you want a marketing/showcase page, put it on a different route.
- **Frontend logger**: use `import { createLogger } from '@/lib/logger'` (or the default `logger` export) instead of `console.*`. `createLogger('scope')` returns a scoped logger with `debug/info/warn/error` and `child(subscope)`. Below `warn` is silenced in production; override per-deploy via `NEXT_PUBLIC_LOG_LEVEL` (`debug|info|warn|error`). Backend uses Nest's built-in `Logger` separately — don't bridge them.
