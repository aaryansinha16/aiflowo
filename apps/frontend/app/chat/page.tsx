...
export default function ChatPage() {
  ...
  React.useEffect(() => {
    log.debug('Auth state', { authLoading, isAuthenticated, hasToken: !!token });
  ...
  const loadChats = async () => {
    ...
    log.debug('loadChats: starting API call');
  ...
  React.useEffect(() => {
    ...
    activeTasks.forEach((taskId) => {
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          log.debug('SSE task update', { data });
          ...
          if (data.status === 'SUCCEEDED') {
            log.info('SSE task completed', { taskId, status: data.status });
            ...
            log.error('SSE task failed', data.error, { taskId });
            setError(`Task failed: ${data.error}`);
            // Remove from active tasks
            setActiveTasks((prev) => prev.filter((id) => id !== taskId));
          }
        } catch (error) {
          log.error('SSE failed to parse event', error);
        }
      };
    ...
    return () => {
      log.debug('SSE cleaning up connections');
