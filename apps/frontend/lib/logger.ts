/**
 * Lightweight client/server logger for the frontend.
 *
 * Use this instead of `console.*` so log output is consistently formatted,
 * level-filtered, and easy to route to an external sink later. Create a scoped
 * logger with `createLogger('scope')` (e.g. `createLogger('auth')`) so messages
 * carry their origin.
 */
...
const formatPrefix = (scope: string | undefined, level: LogLevel): string => {
  ...
  return `${level.toUpperCase()} ${tag}`.trim();
};
...
const buildLogger = (scope?: string): Logger => {
  ...
  return {
    ...
    error: (message, error, meta) => {
      const payload =
        error !== undefined || meta !== undefined ? { error, ...(meta ?? {}) } : undefined;
